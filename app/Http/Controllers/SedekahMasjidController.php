...
class SedekahMasjidController extends Controller
{
    ...
    public function createTransaction(Request $request)
    {
        ...
        $trx = SedekahTransaction::create([
            ...
            'nama' => $data['nama'] ?? (auth()->user()->name ?? 'Hamba Allah'),
            'email' => $data['email'] ?? (auth()->user()->email ?? 'no-reply@example.com'),
        ...
        $itemName = $trx->campaign?->judul
            ? ('Sedekah Masjid - ' . $trx->campaign->judul)
            : 'Sedekah Masjid (Umum)';
