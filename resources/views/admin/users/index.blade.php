...
<div class="mx-auto w-full max-w-7xl">
  ...
  <tr class="border-b border-slate-200 text-left">
    ...
    <th class="px-4 py-3 text-xs font-semibold text-slate-600">No. HP</th>
  ...
  @forelse(($users ?? []) as $u)
    @php
      // rapihin nomor buat WA (hapus spasi, tanda +, strip, dll)
      $rawPhone = (string) ($u->phone ?? '');
      $waPhone = preg_replace('/[^0-9]/', '', $rawPhone);

      // kalau mulai 0, ubah ke 62 (format umum WA Indonesia)
      ...
      <td class="px-4 py-3 text-slate-700 whitespace-nowrap">
        @if(!empty($rawPhone))
          <div class="flex items-center gap-2">
            <span class="font-medium">{{ $rawPhone }}</span>
